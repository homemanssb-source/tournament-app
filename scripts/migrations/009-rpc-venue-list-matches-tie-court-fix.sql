-- ============================================================
-- 009: rpc_venue_list_matches — 단체전 court 이름 매핑 버그 수정
--
-- 배경 (현장 경기관리 코트 배정 기능 추가 중 발견):
--   기존 RPC는 ties.court_number → '코트 ' || court_number 라는
--   리터럴 문자열을 반환했음 (예: "코트 5")
--   venues.courts 배열은 실제 이름(예: "한라-1", "한라-2"...)
--   → 두 값이 절대 일치 안 함 → 단체전이 venue에 배정돼도 반환 안 됨
--
-- 증상:
--   현장 경기관리에서 단체전을 코트에 배정 → 하단 코트 현황에 안 보임
--   운영자 대시보드는 DB를 직접 조회해서 영향 없음
--
-- 해결:
--   WITH 절에서 venues를 created_at 순으로 펼쳐 글로벌
--   court_number → court_name 매핑(jsonb)을 만들어
--   court 컬럼 계산과 필터 양쪽에서 사용
--
-- 실행일: 2026-04-16
-- ============================================================
...
BEGIN
  ...
  -- 글로벌 court_number → court_name 매핑 생성
  -- venues를 created_at 순으로 펼쳐 대시보드와 동일한 인덱스 부여
  ...
  FROM (
    ...
    WHERE t.event_id = v_session.event_id
      AND t.is_bye = false
      AND (
        -- ✅ 글로벌 맵으로 변환된 이름이 venue.courts에 있는지
