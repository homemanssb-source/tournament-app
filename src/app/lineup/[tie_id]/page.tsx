...
export default function LineupPage() {
  ...
  async function loadRevealedData(tid: string, ca: Club | null, cb: Club | null) {
    ...
    if (ca) { (await fetchClubMembers(ca.id)).forEach(m => { memberMap[m.id] = m; }); }
    if (cb) { (await fetchClubMembers(cb.id)).forEach(m => { memberMap[m.id] = m; }); }
    setAllMembers(memberMap);
  }

  async function loadRubbers(tid: string) {
    setRubbers(await fetchRubbers(tid));
  }
  ...
  async function handleScoreSave() {
    ...
    if (setsPerRubber === 3) {
      ...
      const setsB = (s1b > s1a ? 1 : 0) + (s2b > s2a ? 1 : 0);
      if (setsA === 1 && setsB === 1 && (!set3a || !set3b)) { setScoreError('세트 동률입니다. 3세트 점수를 입력하세요.'); return; }
    }
      ...
      <div className="max-w-lg mx-auto p-6 space-y-6">
        ...
        {step === 'pin' && (
          <div className="bg-white rounded-xl border p-6 space-y-4">
            ...
            <input type="tel" maxLength={6} value={pinInput}
            ...
            <button onClick={handlePinSubmit}
              className="w-full bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700">확인</button>
          </div>
        )}
        ...
        {step === 'revealed' && (
          <div className="space-y-4">
            {tie && (
              <div className={`rounded-xl p-4 text-center ${tieCompleted ? 'bg-purple-50 border border-purple-200' : 'bg-blue-50 border border-blue-200'}`}>
                {tieCompleted ? (
                  <>
                    <div className="text-3xl mb-2">🏆</div>
                    <p className="font-bold text-purple-800">대전 완료!</p>
                    <div className="text-2xl font-bold mt-2">
                      <span className={tie.club_a_rubbers_won > tie.club_b_rubbers_won ? 'text-blue-600' : ''}>{clubA?.name}</span>
                      <span className="text-gray-400 mx-2">{tie.club_a_rubbers_won} - {tie.club_b_rubbers_won}</span>
                      <span className={tie.club_b_rubbers_won > tie.club_a_rubbers_won ? 'text-blue-600' : ''}>{clubB?.name}</span>
                    </div>
                  </>
                ) : (
                  <>
              ...
              return (
                <div key={num} className={`bg-white rounded-xl border p-4 ${isCompleted ? 'border-green-200' : ''}`}>
                  <div className="text-sm font-medium text-gray-500 mb-3 text-center flex items-center justify-center gap-2">
                    복식 {num}
                    {isCompleted && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">완료</span>}
                  </div>
                  <div className="grid grid-cols-5 items-center gap-2 mb-3">
                    <div className="col-span-2 text-right text-sm">
                      <div className="font-medium">{getMemberName(lineupA?.player1_id || '')}</div>
                      <div className="font-medium">{getMemberName(lineupA?.player2_id || '')}</div>
                    ...
                    <div className="col-span-2 text-left text-sm">
                      <div className="font-medium">{getMemberName(lineupB?.player1_id || '')}</div>
                      <div className="font-medium">{getMemberName(lineupB?.player2_id || '')}</div>
                  ...
                  {hasScore && !isScoring && (
                    <div className="text-center py-2 bg-gray-50 rounded-lg">
                      ...
                      {rubber!.winning_club_id && (
                        <div className="text-xs text-blue-600 mt-1">승: {rubber!.winning_club_id === clubA?.id ? clubA?.name : clubB?.name}</div>
                      )}
                    </div>
                  )}
                  ...
                  {!hasScore && !isScoring && !tieCompleted && rubber && (
                    <button onClick={() => startScoring(rubber)}
                      className="w-full bg-blue-50 text-blue-700 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-100">점수 입력</button>
                  )}
                    ...
                    <div className="space-y-3 mt-2 border-t pt-3">
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button onClick={() => setSelectedWinner('a')}
                            className={`py-3 rounded-lg text-sm font-bold transition ${selectedWinner === 'a' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                            {clubA?.name}</button>
                          <button onClick={() => setSelectedWinner('b')}
                            className={`py-3 rounded-lg text-sm font-bold transition ${selectedWinner === 'b' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                            {clubB?.name}</button>
                        </div>
                      </div>
                      {selectedWinner && (
                        <>
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => { setScoringRubber(null); setScoreError(''); }}
                              className="flex-1 bg-gray-100 py-2.5 rounded-lg text-sm">취소</button>
                            <button onClick={handleScoreSave} disabled={scoreSaving}
                              className="flex-1 bg-green-600 text-white py-2.5 rounded-lg text-sm font-bold hover:bg-green-700 disabled:opacity-50">
            ...
            {tieCompleted && (
              <div className="bg-gray-100 rounded-xl p-3 text-center text-sm text-gray-500">🔒 대전 완료 — 점수 수정은 운영본부에서만 가능합니다.</div>
            )}
          </div>
        )}
      </div>
