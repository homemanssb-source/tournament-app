...
function SyncDashboardInner() {
  ...
  async function handleConnect() {
    if (!appAEventId.trim()) return alert('앱A 대회 ID를 입력하세요.');
    await supabase.from('events').update({
      app_a_event_id: appAEventId.trim(),
  ...
  async function handlePullTeam() {
    ...
    try {
      ...
      setSyncResult({ type: 'team', ...result });
      await loadEventData();
    } catch (err: any) {
      setSyncResult({ type: 'team', success: false, error: err.message });
    } finally {
      setSyncing(false);
    }
  }
  ...
  async function handlePullIndividual() {
    ...
    try {
      ...
      const result = await res.json();
      setSyncResult({ type: 'individual', ...result });
      await loadEventData();
    } catch (err: any) {
      setSyncResult({ type: 'individual', success: false, error: err.message });
    } finally {
      setSyncing(false);
    }
  }
        ...
        <>
            ...
            {event?.app_a_connected ? (
              <div className="bg-green-50 rounded-lg p-4">
                <p className="text-green-800 text-sm">
                  ✅ 연결됨 — 앱A 대회 ID: <code className="bg-green-100 px-2 py-0.5 rounded">{event.app_a_event_id}</code>
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-sm text-gray-500">
                  앱A의 대회 ID(event_id)를 입력하여 연결하세요.
                  앱A Supabase 대시보드 → events 테이블에서 확인 가능합니다.
                </p>
                <div className="flex gap-2">
                  <input
                    value={appAEventId}
                    onChange={e => setAppAEventId(e.target.value)}
                    placeholder="앱A event_id (UUID)"
          ...
          {syncLogs.length > 0 && (
            <div className="bg-white rounded-lg border p-6">
              ...
              <p className="text-xs text-gray-400 mb-3">마지막 동기화: {lastSync}</p>
                  ...
                  <div key={log.id} className="flex items-center justify-between py-2 text-sm">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-0.5 rounded ${
                        ...
                        {log.sync_type === 'team' ? '단체전' : '개인전'}
                      </span>
                      <span className="text-gray-600">{log.app_b_table}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs ${log.status === 'synced' ? 'text-green-600' : 'text-red-600'}`}>
                        {log.status === 'synced' ? '✅' : '❌'} {log.status}
                      </span>
