...
//
// event_type에 따라:
// - individual: 기존 개인전 탭 (조게시/토너먼트/경기결과/코트현황)
// - team: 단체전 탭 (순위/대전/토너먼트/코트현황)
// - both: 개인전/단체전 모드 전환
// ============================================================
'use client'
...
import { fetchClubs, fetchTies, fetchStandings, fetchEventTeamConfig, fetchRevealedLineups, fetchClubMembers, fetchRubbers } from '@/lib/team-api'
import { getFormatLabel, getRoundLabel, getTieStatusLabel, getTieStatusColor, formatSetScore, getMajority } from '@/lib/team-utils'
import type { Club, TieWithClubs, StandingWithClub, EventTeamConfig, TeamLineup, ClubMember, TieRubber } from '@/types/team'
...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setEventType(type)
      setMode(type === 'individual' ? 'individual' : 'team')
        ...
        {eventType === 'both' && (
          <div className="max-w-5xl mx-auto px-4 flex gap-2 pb-2">
            <button onClick={() => setMode('individual')}
              className={`px-4 py-1.5 rounded-full text-sm font-medium ${mode === 'individual' ? 'bg-white text-[#2d5016]' : 'bg-white/20 text-white/80'}`}>
              🎾 개인전</button>
            <button onClick={() => setMode('team')}
              className={`px-4 py-1.5 rounded-full text-sm font-medium ${mode === 'team' ? 'bg-white text-[#2d5016]' : 'bg-white/20 text-white/80'}`}>
              📋 단체전</button>
          </div>
        )}
          ...
          <div key={key} className="bg-white rounded-xl border overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead className="border-t bg-gray-50">
                <tr><th className="px-3 py-2 text-left w-12">#</th><th className="px-3 py-2 text-left">클럽</th><th className="px-3 py-2 text-center w-10">승</th><th className="px-3 py-2 text-center w-10">패</th><th className="px-3 py-2 text-center w-14">득실</th></tr>
              </thead>
        ...
        <div key={tie.id} className={`bg-white rounded-xl border p-4 ${tie.is_bye ? 'opacity-50' : ''}`}>
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-gray-400">{getRoundLabel(tie.round || '')} #{tie.tie_order}
              {tie.court_number && <span className="ml-2 text-green-700 font-medium">코트 {tie.court_number}</span>}
            </span>
          ...
          {tie.status === 'lineup_phase' && (
            <div className="text-xs text-center text-yellow-600 mt-2">
              라인업: {tie.club_a_lineup_submitted ? '✅' : '⏳'} {tie.club_a?.name} · {tie.club_b_lineup_submitted ? '✅' : '⏳'} {tie.club_b?.name}
            </div>
          )}
        </div>
      ))}
    ...
    <div className="space-y-4">
              ...
              <div key={tie.id} className="p-3">
                <div className="flex items-center">
                  <div className={`flex-1 ${tie.winning_club_id === tie.club_a_id ? 'font-bold' : ''}`}>
                    {tie.club_a?.seed_number && <span className="text-xs text-yellow-600 mr-1">[{tie.club_a.seed_number}]</span>}
                    {tie.club_a?.name || (tie.is_bye ? 'BYE' : 'TBD')}
                  </div>
                  <div className="px-4 font-bold">
                    {tie.status === 'completed' || tie.status === 'in_progress' ? `${tie.club_a_rubbers_won} - ${tie.club_b_rubbers_won}` : tie.is_bye ? 'BYE' : 'vs'}
                  </div>
                  <div className={`flex-1 text-right ${tie.winning_club_id === tie.club_b_id ? 'font-bold' : ''}`}>
                    {tie.club_b?.name || (tie.is_bye ? 'BYE' : 'TBD')}
                    {tie.club_b?.seed_number && <span className="text-xs text-yellow-600 ml-1">[{tie.club_b.seed_number}]</span>}
                  </div>
                </div>
      ...
      {winner && (
        <div className="bg-yellow-50 border-2 border-yellow-300 rounded-xl p-6 text-center">
          <div className="text-4xl mb-2">🏆</div><div className="text-2xl font-bold">{winner}</div><div className="text-sm text-yellow-700 mt-1">우승!</div>
        </div>
      )}
    </div>
  )
...
function TeamCourtsView({ ties }: { ties: TieWithClubs[] }) {
  ...
  if (courtTies.length === 0 && unassigned.length === 0) {
    return <div className="text-center text-gray-400 py-8">코트 배정 정보가 없습니다.</div>
  }
